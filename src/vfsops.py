...
class FileInfo(pyfuse3.EntryAttributes):
...
class VFS:
	...
	def __init__(self, sourceDir: FileInfo, cacheDir: FileInfo):
		self._inode_path_map = {pyfuse3.ROOT_INODE: FileInfo(sourceDir)}
		self._inode_path_cache = {pyfuse3.ROOT_INODE: FileInfo(cacheDir)}
	...
	def _add_path(self, inode, path, fromPopulate=False):
		...
		if inode not in self._inode_path_map:
			self._inode_path_cache[inode] = CachePath.toCachePath(path)
			self._inode_path_map[inode] = CachePath.toSrcPath(path)
			return
...
class VFSOps(pyfuse3.Operations):
	...
	async def __readdir(self, path, off, token):
		...
		for name in os.listdir(path):
			if name == '.' or name == '..':
				continue
			attr = FileInfo.getattr(path=os.path.join(path, name))
