...
def test_init_disk(tmpdir_factory):
...
def test_toPathFuncs(tmpdir_factory):
	tmpdir_source, tmpdir_cache = get_src_cache_directory_pair(tmpdir_factory)
	disk = prep_Disk(tmpdir_source, tmpdir_cache)
...
def test_canStore(tmpdir_factory):
	tmpdir_source, tmpdir_cache = get_src_cache_directory_pair(tmpdir_factory)
...
def test_copystat(tmpdir_factory):
...
@pytest.mark.skip
def test_mkdir_p(tmpdir_factory):
	tmpdir_source, tmpdir_cache = get_src_cache_directory_pair(tmpdir_factory)
	disk = prep_Disk(tmpdir_source, tmpdir_cache, maxCacheSize=1)
...
def test_path2Ino(tmpdir_factory):
	tmpdir_source, tmpdir_cache = get_src_cache_directory_pair(tmpdir_factory)
	disk = prep_Disk(tmpdir_source, tmpdir_cache, maxCacheSize=1)
